...
def mqtt_message(client, userdata, message):
    ...
    match message.topic.split('/')[-1]:
        case 'r1':
            ...
            mqttClientRelays.publish(MQTT_TOPIC_PUB + relay_topics[0], str(response) == '255')
            print(MQTT_TOPIC_PUB + relay_topics[0])
        case 'r2':
            response = setRelay(payload == '1', 2)
            print('Response R2: ' + str(response))
            mqttClientRelays.publish(MQTT_TOPIC_PUB + relay_topics[1], str(response) == '255')
            print(MQTT_TOPIC_PUB + relay_topics[1])
        case 'r3':
            response = setRelay(payload == '1', 3)
            print('Response R3: ' + str(response))
            mqttClientRelays.publish(MQTT_TOPIC_PUB + relay_topics[2], str(response) == '255')
